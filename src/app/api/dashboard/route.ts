...
import { db } from "@/db";
import { users, checkins, lessons, lessonCompletions, posts, liveEvents, reservations, challenges, challengeProgress, weeklyPhrases, dailyPrompts } from "@/db/schema";
...
export async function GET(req: Request) {
  ...
  if (!me) return NextResponse.json({ error: "no users, seed first" }, { status: 404 });
  const uid = me.id;
  ...
  const labels = ["S", "T", "Q", "Q", "S", "S", "D"]; // Seg..Dom but display D S T Q Q S S? Use PT week S..D
  // We'll show D S T Q Q S S starting Sunday like original
  ...
  // week checkins count for "dias de presença"
